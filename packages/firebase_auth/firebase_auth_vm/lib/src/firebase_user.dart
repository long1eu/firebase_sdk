...
typedef UpdateSetAccountInfoRequest = void Function(
    gitkit.UserInfo user, SetAccountInfoRequest request);

class FirebaseUser implements UserInfo {
  ...
  static Future<FirebaseUser> _retrieveUserWithAuth(
    ...
    return user
      ...
      .._updateWithGetAccountInfoResponse(response);
  }
  ...
  late Map<String, UserInfo> _providerData;
  ...
  /// Whether or not the user can be authenticated by using Firebase email and
  /// password.
  late bool _hasEmailPasswordCredential;
  ...
  /// them to revoke the email address change, in order to protect them from
  /// account hijacking.
  ///
  /// May fail if there is already an account with this email address that was
  ...
  ///   Indicates the email is already in use by another account.
  ///   * [FirebaseAuthError.invalidEmail]
  ///   Indicates the email address is malformed.
  ///   * [FirebaseAuthError.requiresRecentLogin]
  ///   Indicates that the user's last sign-in time does not meet the security
  ///   threshold. Use reauthenticate methods to resolve.
  ...
  ///
  /// Anonymous users who update both their email and password will no longer be
  /// anonymous. They will be able to log in with these credentials.
  ///
  /// Errors:
  ...
  ///
  /// The new phone number credential corresponding to the phone number to be
  /// added to the Firebase account, if a phone number is already linked to the
  ...
  Future<void> updateProfile(UserUpdateInfo userUpdateInfo) async {
    return _runUserUpdateTransaction(
      (_, SetAccountInfoRequest request) {
  ...
  /// [credential]s supplied by the user and returns additional identity
  /// provider data.
  ///
  /// This is used to prevent or resolve [FirebaseAuthError.requiresRecentLogin]
  ...
  ///   Indicates that accounts with the identity provider represented by the
  ///   credential are not enabled. Enable them in the Auth section of the
  ///   Firebase console.
  ///   * [FirebaseAuthError.emailAlreadyInUse]
  ...
  ///   cannot be authenticated with this method. Call
  ///   [FirebaseAuth.fetchSignInMethodsForEmail] for this user’s email and then
  ///   prompt them to sign in with any of the sign-in providers returned. This
  ///   error will only be thrown if the "One account per email address" setting
  ///   is enabled in the Firebase console, under Auth settings. Please note
  ///   that the error code raised in this specific situation may not be the
  ///   same on Web and Android.
  ///   * [FirebaseAuthError.userDisabled]
  ///   Indicates the user's account is disabled.
  ///   * [FirebaseAuthError.wrongPassword]
  ///   Indicates the user attempted reauthentication with an incorrect
  ///   password, if credential is of the type [EmailPasswordAuthCredential].
  ///   * [FirebaseAuthError.userMismatch]
  ...
  /// desired.
  ///
  /// Useful when authenticating against your own backend. Use our server SDKs
  /// or follow the official documentation to securely verify the integrity and
  ...
  ///
  /// This allows the user to sign in to this account in the future with the
  /// given account.
  ///
  /// Errors:
  ///   * [FirebaseAuthError.weakPassword]
  ...
  ///   Indicates the supplied credential is invalid. This could happen if it
  ///   has expired or it is malformed.
  ///   * [FirebaseAuthError.credentialAlreadyInUse]
  ...
  ///   Indicates an attempt to link a provider of a type already linked to this
  ///   account.
  ///   * [FirebaseAuthError.operationNotAllowed]
  ///   Indicates that accounts with the identity provider represented by the
  ///   credential are not enabled. Enable them in the Auth section of the
  ...
  ///   Indicates that the action code in the link is malformed, expired, or has
  ///   already been used. This can only occur when using
  ///   [EmailAuthProvider.credentialWithLink] to obtain the credential.
  Future<AuthResult> linkWithCredential(AuthCredential credential) async {
    if (_providerData.containsKey(credential.providerId)) {
    ...
    try {
      ...
      final AdditionalUserInfo additionalUserInfo =
          AdditionalUserInfo.fromVerifyAssertionResponse(response);
  ...
  /// Disassociates a user account from a third-party identity provider with
  /// this user.
  ///
  ...
  Future<void> unlinkFromProvider(String providerId) async {
    ...
    final String accessToken = await _getToken();
    final SetAccountInfoRequest request = SetAccountInfoRequest()
      ..idToken = accessToken
  ...
  /// Send an email to verify the ownership of the account then update to the
  /// new email.
  ///
  /// Errors:
  ///   * [FirebaseAuthError.invalidRecipientEmail]
  ...
  ///   * [FirebaseAuthError.userNotFound]
  ///   Indicates the user account was not found.
  Future<void> sendEmailVerificationBeforeUpdating(String newEmail,
      [ActionCodeSettings? settings]) async {
    final String accessToken = await _getToken();
  ...
  ///
  /// This method is specifically for providing the access token to internal
  /// clients during deserialization and sign-in events, and should not be used
  /// to retrieve the access token by anyone else.
  ...
  DateTime get _accessTokenExpirationDate {
    return _secureTokenApi._accessTokenExpirationDate;
  }
  ...
  Future<void> _updateEmailAndPassword(String? email, String? password) async {
    ...
    if (this.email != null && !hadEmailPasswordCredential) {
      // The list of providers need to be updated for the newly added
      ...
      try {
        ...
        for (gitkit.UserInfo userAccountInfo in response.users!) {
          // Set the account to non-anonymous if there are any providers, even
          // if they're not email/password ones.
  ...
  Future<void> _linkWithEmailLink(
    EmailPasswordAuthCredential credential,
  ) async {
    final String accessToken = await _getToken();
    final Uri link = Uri.parse(credential.link!);
    final String actionCode = link.queryParameters['oobCode']!;

    final EmailLinkSigninRequest request = EmailLinkSigninRequest()
  ...
  /// Links a game center account with this user. On success, the cached
  ...
  /// Performs a setAccountInfo request by mutating the results of a
  /// getAccountInfo response, atomically in regards to other calls to this
  /// method.
  Future<void> _runUserUpdateTransaction(
    ...
    return _runner.enqueue<void>(() async {
      ...
      final SetAccountInfoRequest request =
          SetAccountInfoRequest(idToken: accessToken);
  ...
  /// Gets the user's account data from the server, updating our local values.
  ...
  void _updateWithGetAccountInfoResponse(GetAccountInfoResponse response) {
    ...
    _metadata = UserMetadata._(
      lastSignInDate: user.lastLoginAt != null
    ...
    _providerData =
        user.providerUserInfo?.asMap().map((_, UserInfoProviderUserInfo info) {
              return MapEntry<String, UserInfo>(
                  info.providerId!,
                  UserInfo._(
