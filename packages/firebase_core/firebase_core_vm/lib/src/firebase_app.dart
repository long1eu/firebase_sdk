...
class FirebaseApp extends _PlatformDependencies {
  ...
  factory FirebaseApp.withOptions(
    ...
    final FirebaseApp firebaseApp =
        FirebaseApp._(normalizedName, options, dependencies) //
  ...
  static FirebaseApp get instance {
    ...
    if (defaultApp == null) {
      throw StateError(
          'Default FirebaseApp is not initialized. Make sure to call [FirebaseApp.withOptions()] first.');
    }
  ...
  static FirebaseApp getInstance(String name) {
    final FirebaseApp? firebaseApp = _instances[name.trim()];
    if (firebaseApp != null) {
      return firebaseApp;
    }
  ...
  static const String _dataCollectionKey =
  ...
  /// Disabled by default.
  void setAutomaticResourceManagementEnabled({bool enabled = false}) {
    _checkNotDeleted();
  ...
  void _checkNotDeleted() {
    checkState(!_deleted, 'FirebaseApp was deleted');
  }
...
abstract class _PlatformDependencies extends PlatformDependencies
  ...
  Stream<InternalTokenResult> get onTokenChanged {
    return const Stream<InternalTokenResult>.empty();
  }
}
