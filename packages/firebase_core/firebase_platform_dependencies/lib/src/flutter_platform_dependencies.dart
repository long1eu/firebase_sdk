...
class FlutterPlatformDependencies extends core.PlatformDependencies
    with WidgetsBindingObserver {
  FlutterPlatformDependencies._(
    this.box,
    this.onBackgroundChanged,
    this.onNetworkConnected,
  );

  static Future<core.PlatformDependencies> initializeForApp(
      [String appName = core.FirebaseApp.defaultAppName]) async {
    ...
    final Box<Uint8List> keyBox =
        await Hive.openBox<Uint8List>('encryption.store', path: args.boxPath);
    if (!keyBox.containsKey('key')) {
      final List<int> key = Hive.generateSecureKey();
      await keyBox.put('key', Uint8List.fromList(key));
    }
    ...
    Connectivity()
      ..checkConnectivity().then((ConnectivityResult event) =>
          onNetworkConnected.add(event != ConnectivityResult.none))
      ..onConnectivityChanged.listen((ConnectivityResult event) =>
          onNetworkConnected.add(event != ConnectivityResult.none));

    final FlutterPlatformDependencies instance = FlutterPlatformDependencies._(
        box, onBackgroundChanged, onNetworkConnected);
  ...
  static core.PlatformDependencies getInstance(
      [String appName = core.FirebaseApp.defaultAppName]) {
    if (_instances.containsKey(appName)) {
      return _instances[appName]!;
    } else {
      throw StateError(
  ...
  static final Map<String, core.PlatformDependencies> _instances =
      <String, core.PlatformDependencies>{};
